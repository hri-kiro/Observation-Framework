...
# Load data
data <- read.csv("..Rdata\\compare_difference.csv")

# Summarize mean and standard deviation for each Gender category
summary_stats <- data %>%
  group_by(Gender) %>%
  summarise(
...
# Mann-Whitney U tests for each variable between gender groups
obs_prox_test <- wilcox.test(Obs_prox ~ Gender, data = data)
obs_int_test <- wilcox.test(Obs_int ~ Gender, data = data)
freq_prox_test <- wilcox.test(Freq_prox ~ Gender, data = data)
freq_int_test <- wilcox.test(Freq_int ~ Gender, data = data)
